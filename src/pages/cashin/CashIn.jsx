...
const CashIn = () => {
  ...
  const generateTransactionId = () =>
    `TXN-${Date.now().toString(36).toUpperCase()}-${Math.floor(Math.random() * 9999).toString().padStart(4, "0")}`;

  const handleNext = () => {
    if (step === 1 && selectedRows.length === 0) { alert("Please select at least one order"); return; }
    if (step === 2 && !selectedRows.some((row) => parseFloat(amounts[row.id]) > 0)) { alert("Please enter amount for at least one order"); return; }
  ...
  const DenomStatusBadge = ({ denomTotal, target }) => {
    ...
    if (matched) return <span className="inline-flex items-center text-xs px-3 py-1 rounded-full bg-green-500/10 border border-green-500/30 text-green-400">Matched</span>;
    if (diff > 0) return <span className="inline-flex items-center text-sm px-3 py-1 rounded-full bg-red-500/10 border border-red-400/30 text-red-400">৳{diff.toFixed(2)} over</span>;
    return <span className="inline-flex items-center text-sm px-3 py-1 rounded-full bg-red-500/10 border border-red-400/30 text-red-400">৳{Math.abs(diff).toFixed(2)} left</span>;
  };
   ...
   *    • Balance = vault balance BEFORE this cash-in was added
   *    • If approved  → opening = vault.current_balance - cashIn amount
   *      (because the vault already includes it; we back it out for the opening line)
   *    • If pending   → opening = vault.current_balance
   *      (vault hasn't changed yet)
   *    • Debit: —   Credit: —
   *
   *  Row 2 — Cash-In Transaction (only shown when approved)
   *    • Credit = cash_in_amount   (money credited INTO the vault)
   *    • Debit: —
   *    • Balance = Opening + cash_in_amount
   *    • Note = Transaction ID
   *
   *  Last Row — Closing Balance
   *    • If approved  → closing = opening + cash_in_amount
   *    • If pending   → closing = opening  (no change yet)
   *    • Debit: —   Credit: —
   */
  const downloadCashInLedger = async (cashIn) => {
    try {
      ...
      const openingBalance = is_approved
        ? vaultBalance - cashInAmount
        : vaultBalance;

      const closingBalance = is_approved
        ? openingBalance + cashInAmount
        : openingBalance;
  ...
  const columnsStep0 = [
    {
      title: "Vault", key: "vault_id", className: "w-14",
      render: (row) => <span className="font-mono text-cyan-400">{row.vault?.vault_id}</span>,
    },
    {
      title: "Bag", key: "bag", className: "w-24",
      render: (row) => <span>{row?.bags?.barcode}-RN{row?.bags?.rack_number}</span>,
    },
    {
      title: "Tran Id", key: "tran_id", className: "w-24",
      render: (row) => <span>{row?.tran_id}</span>,
    },
    {
      title: "Order Ids", key: "orders", className: "w-[150px]",
      render: (row) => (
        <div className="flex flex-wrap gap-2">
          {row?.orders?.map((order, i) => <span key={i}>{order?.order_id},</span>)}
        </div>
      ),
    },
    {
      title: "Amount", key: "cash_in_amount", className: "w-10",
      render: (row) => <span>{row?.cash_in_amount}</span>,
    },
    {
      title: "Req at", key: "created_at", className: "w-20",
      render: (row) => <span>{dayjs(row.created_at).format("DD MMM, YYYY hh:mm A")}</span>,
    },
    {
      title: "Verifiers", key: "required_verifiers", className: "w-20",
      render: (row) => <VerifierAvatars requiredVerifiers={row.required_verifiers || []} />,
    },
    {
      title: "Approvers", key: "required_approvers", className: "w-20",
      render: (row) => <VerifierAvatars requiredVerifiers={row.required_approvers || []} />,
    },
    {
      title: "Verifier status", key: "verifier_status", className: "w-20",
      render: (row) => (
    ...
    {
      title: "Approver status", key: "status", className: "w-20",
      render: (row) => (
        <span className={`capitalize text-xs px-2.5 py-1 rounded-full border ${
    ...
    {
      title: "Action", key: "actions", className: "w-24",
      render: (row) => {
        ...
        return (
          <div className="flex items-center gap-3 py-2">
            ...
            <motion.button
              whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
              onClick={(e) => { e.stopPropagation(); if (window.confirm("Delete?")) {} }}
  ...
  const columnsStep1 = [
    ...
    { title: "Phone",           key: "customer_phone",      className: "w-28", render: (row) => <span>{row?.customer_phone}</span> },
    ...
    {
      title: "Select", key: "selection", className: "w-40",
      render: (row) => {
          ...
          <div className="flex items-center justify-center">
            ...
            >
              ...
              <motion.svg viewBox="0 0 24 24" fill="none" className="w-4 h-4 relative z-10 pointer-events-none" initial={false}>
                <motion.path
                  d="M4 12L9 17L20 6" stroke="blue" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"
  ...
  const columnsStep2 = [
    ...
    {
      title: "Cash To Vault", key: "amount", className: "w-40",
      render: (row) => (
        <input
          readOnly type="number" min="0" step="0.01" placeholder="0.00"
  ...
  const stepDes   = ["Manage Your All Cash In", "Select orders to cash in", "Select orders to cash in", "Select denominations to cash in", "Cash In Complete"][step] ?? "";
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-4">
          {step >= 1 && (
            <div onClick={handleBack} className="cursor-pointer transition-all px-4 py-1 hover:bg-gray-100 rounded-lg text-zinc-500 hover:text-black border border-zinc-100">
              Back
            </div>
          )}
          {step === 0 && !isLocked && (
          ...
          {step === 1 && selectedRows.length > 0 && (
            <div onClick={handleNext} className="cursor-pointer transition-all px-4 py-1 hover:bg-white/10 rounded-lg text-cyan-300 hover:text-cyan-600 bg-cyan-50 border border-cyan-300">
              Next
            </div>
          )}
          {step === 2 && (
            <div onClick={handleNext} className="cursor-pointer transition-all px-4 py-1 rounded-lg bg-cyan-500 text-white border border-cyan-500/50 hover:text-gray-600">
              Next
            </div>
          )}
          {step === 3 && totalDenominationAmount === totalEnteredAmount && (
            <div onClick={handleFinish} className="cursor-pointer transition-all px-4 py-1 hover:bg-black duration-300 ease-in-out rounded-lg text-white bg-[#424242] border border-cyan-500/50">
              Finish
            </div>
          )}
        </div>
      ...
      {step === 0 && (
        <DataTable columns={columnsStep0} data={cashIns} changePage={handlePageChange} onSearch={handleSearch} paginationData={paginationData} selectedRows={selectedRows} loading={loading} setSelectedRows={setSelectedRows} className="h-[calc(100vh-120px)]" />
      )}
      {step === 1 && (
        <DataTable columns={columnsStep1} data={orders} changePage={handlePageChange} onSearch={handleSearch} paginationData={paginationData} selectedRows={selectedRows} loading={loading} setSelectedRows={setSelectedRows} className="h-[calc(100vh-100px)]" />
      )}
      {step === 2 && (
        <div>
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            ...
            <div className="h-1.5 rounded-full bg-gray-300 overflow-hidden">
              <div
                ...
                style={{ width: totalEnteredAmount > 0 ? `${Math.min(100, (totalDenominationAmount / totalEnteredAmount) * 100)}%` : "0%" }}
              />
            </div>
            <div className="grid grid-cols-5 gap-3">
              {DENOM_NOTES.map((n) => {
                ...
                return (
                  <div key={n} className={`flex flex-col items-center gap-1.5 rounded-lg p-3 border transition-all ${hasValue ? "bg-[#E1F5EE]/50 border-green-500" : "bg-[#F5F4ED] border-zinc-700/40"}`}>
                    ...
                    <input
                      ...
                      value={count > 0 ? count : ""}
                      className={`w-full text-center py-1 rounded-md text-base font-medium bg-white border outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none ${hasValue ? "text-black border-gray-400" : "text-zinc-300"}`}
                      onChange={(e) => {
                        const raw = e.target.value.replace(/[^0-9]/g, "");
                        setDenominations((prev) => ({ ...prev, [n]: parseInt(raw) || 0 }));
                      }}
            ...
            <div className="grid grid-cols-5 gap-2">
              {DENOM_NOTES.map((n) => {
                const count = parseInt(denominations[n]) || 0;
                return (
                  <div key={n} className={`text-center text-sm py-1.5 flex flex-col rounded border ${count > 0 ? "bg-[#E1F5EE]/50 border-green-500 text-green-800" : "bg-transparent border-zinc-700/30 text-zinc-600"}`}>
      ...
      {step === 4 && transactionId && (
        <div className="max-w-2xl mx-auto text-center">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-zinc-800/80 backdrop-blur-xl border border-cyan-500/30 rounded-2xl p-10">
            ...
            <div className="flex justify-center mb-10">
              <canvas ref={(canvas) => {
                if (canvas && transactionId) {
                  JsBarcode(canvas, transactionId, { format: "CODE128", width: 2.5, height: 100, displayValue: true, fontSize: 20, textMargin: 10, font: "monospace", background: "#18181b", lineColor: "#67e8f9", margin: 20 });
                }
              }} />
            </div>
            <div className="text-left bg-zinc-900/80 rounded-xl p-6 mb-8">
              <p className="text-lg">Orders: <strong>{selectedRows.length}</strong></p>
              <p className="text-lg">Total Amount: <strong className="text-cyan-400">৳{totalEnteredAmount.toFixed(2)}</strong></p>
              <p className="text-sm text-zinc-400 mt-4">Denominations:</p>
              {Object.entries(denominations).filter(([, count]) => count > 0).map(([note, count]) => (
                <p key={note} className="text-sm">{note} TK × {count} = ৳{(note * count).toLocaleString()}</p>
              ))}
            </div>
